...
#        self.port_to_eeprom2_mapping = {}
        ...
        for x in range(self.PORT_START, self.PORT_END + 1):
            ...
            self.port_to_eeprom_mapping[p_num] = eeprom_path.format(self.port_to_i2c_mapping[p_num])
    ...
    def get_presence(self):
        ...
        """
        if self.index < self.PORT_START or self.index > self.PORT_END:
            return False

        cpld_i = self.get_cpld_num(self.index)

        cpld_ps = self._cpld_mapping[cpld_i]
        if self.index > 15:
            port_num = self.index - 16
        else:
            port_num = self.index
